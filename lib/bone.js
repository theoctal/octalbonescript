...
module.exports = {
    ...
    getpin: function(pin) {
        if (typeof pin == 'object') return (pin);
            ...
            if (pinmap[pin].exists) {
				...
				if (typeof pinmap[pin].ain != 'undefined' ||
                    typeof pinmap[pin].led != 'undefined') {
						pinmap[pin].exists = true;
						return pinmap[pin];
						}
                var p = pinmap[pin].key + "_pinmux";
                var path = module.exports.find_sysfsFile(p, module.exports.is_ocp(), 'ocp:' + p );// was (p, module.exports.is_ocp(), p + '.')
				debug('path to pin = ' + path);
	...
	is_cape_analog: function(callback) {
        var ocp = module.exports.is_ocp();
        debug('is_ocp() = ' + ocp);
        var cape_universal = module.exports.find_sysfsFile('44e0d000.tscadc', ocp, '44e0d000.tscadc', callback);
        debug('is_cape_analog() = ' + cape_universal);
        return (cape_universal);
    },
    ...
    file_find: function(path, prefix, callback) {
		...
		debug('file_find path = ' + path + ' --> prefix = ' + prefix);
        if (callback) {
    ...
    create_dt_sync: function(template, load, force_create) {
        ...
        function createDTS() {
            var templateFilename = require.resolve('octalbonescript_capemgr4_1').replace('index.js',
    ...
    load_dt_sync: function(name) {
        ...
        function onFindCapeMgr(x) {
			...
			console.log('Slots file = ' + slotsFile);
            readSlots();
        }
